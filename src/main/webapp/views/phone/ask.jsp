...
<body class="wap_hotquestion_all wap_hotquestion_all2 refer_list_body">
	<div class="wap_container">
				...
				<section class="hotquestion_container" data-title="楼盘咨询正文">
					<div class="hotquestion_info_ask border_e9e9e9">
						<p class="its_writer_time">
							${questionInfo.userName }<span class="h_space2"><fmt:formatDate value="${questionInfo.createTime }" pattern="yyyy年M月d日" /></span>
						</p>
					...
					<div class="hotquestion_info_answer">
						<c:choose>
							...
							<p class="its_writer_time">
								${questionInfo.editorDesc }<span class="h_space2"><fmt:formatDate value="${questionInfo.updateTime }" pattern="yyyy年M月d日" /></span>
							</p>
							<p class="answer_paragram">答：${questionInfo.answer }</p>
							</c:when>
							<c:otherwise><p class="answer_paragram">${questionInfo.answer }</p></c:otherwise>
						</c:choose>
					</div>
				</section>

				<section class="hotquestion_container" data-title="相关楼盘">
					<header class="public_title1 public_title1_noborder"
						data-title="相关楼盘-标题">
						<h2 class="its_tit1">
							相关楼盘<em class="its_icon1"></em>
						</h2>
					</header>
					<ul class="search_list_container" data-title="搜索结果-列表">
						<li><a
							href="${ctx }/${_city.cityPinyinAbbr }/loupan/${bl.groupId}/"
							role="text" url="" class="clearfix">
								<div class="fl its_img">
									<img alt="${bl.projName }" src="${bl.url }">
								</div>
								<div class="its_content">
									...
									<c:if test="${ not empty fn:trim(bl.discount)}">
										<p class="it_zhekou">
											<em class="zhekou1"></em>${bl.discount}
										</p>
									</c:if>
								</div>
						</a></li>
		...
		<!-- 面包屑 -->
		<div class="link_boxs">
			<a href="/${_city.cityPinyinAbbr }/">新房</a> <span class="h_space1">></span><a
				href="${ctx }/${_city.cityPinyinAbbr }/loupan/">${_city.cityName
				}楼盘</a> <span class="h_space1">></span><a
				href="${ctx }/${_city.cityPinyinAbbr }/loupan/${bl.groupId}/"> <c:choose>
			...
			</a> <span class="h_space1">></span>楼盘咨询
		</div>
		<!-- 400电话 -->
	...
	<div class="alert_box" id="check_net">
		...
		<div class="itos_btn1" id="check_net_confirm">
