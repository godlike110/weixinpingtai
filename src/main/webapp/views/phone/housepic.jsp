...
<title>${info.projName }-${_city.cityName }${info.projName }楼盘图片信息-搜狐焦点</title>
<meta name="Keywords" content="${info.projName }-${_city.cityName }${info.projName }楼盘图片信息-搜狐焦点">
...
<body class="wap_apartmentpicture_all wap_housepicture_all">
	<div class="wap_container">
		...
		<c:if test="${returnFlag=='1' }">
			...
			<span class="search_title">楼盘图片</span>
			...
			<ul class="nav_box_ul1 apartmentpicture_menu_ul1">
				<li><a href="#" role="text" url="" role-current="current" class="current first">全部图片</a></li>
				<c:forEach var="pics" items="${data}">
				<li><a href="#" role="text" url="">${pics.minMaxArea}</a></li>
				</c:forEach>
			</ul>
		</header>
		...
		<section class="wap_main apartmentpicture_main">			
			<section class="wap_module apartment_picture_container" data-title="楼盘图片-模块-内容">
				<div class="loupan_img_container" id="scroller">
					<ul class="loupan_img_ul">
					<c:forEach var="pics" items="${data}">
					<c:if test="${deftype==pics.type }">
					<c:forEach var="photo" items="${pics.photoes}">
			...
			<!-- 面包屑 -->
			<div class="link_boxs"><a href="${ctx}/${_city.cityPinyinAbbr}/">新房</a><span class="h_space1">&gt;</span><a href="${ctx}/${_city.cityPinyinAbbr}/loupan/"> ${_city.cityName }楼盘</a><span class="h_space1">&gt;</span><a href="${ctx}/${_city.cityPinyinAbbr}/loupan/${info.groupId}/"> ${buildName}</a><span class="h_space1">&gt;</span> 楼盘图片</div>
	...
	<script type="text/javascript" src="http://192.168.242.44/sceapp/focus_static/wap/phone/snippets/wap_house_picture_v1.0.js"></script>
