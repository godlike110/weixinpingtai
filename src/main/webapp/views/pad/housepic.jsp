	...
	<body>
		...
		<c:if test="${returnFlag=='1' }">
			<a href="${ctx}/${_city.cityPinyinAbbr}/" class="back" data-title="返回上一级">首页</a>
				...
				<h1>楼盘图片</h1>
				<div class="menu menu_right image_menu" id="menu">
					<a href="javascript:;" class="filter">筛选</a>
					<div class="menu_list">
						<a href="javascript:;" role="touch_bg" data-type="11111">全部图片</a>
						<c:forEach var="pics" items="${data}">
						<a href="javascript:;" role="touch_bg" data-type="${pics.type}">${pics.minMaxArea}</a>
						</c:forEach>
					</div>
				</div>
			</header>
			...
			<section class="crumb">
				<div class="inner">
					<a href="${ctx}/${_city.cityPinyinAbbr}/">新房</a>&gt;<a href="${ctx}/${_city.cityPinyinAbbr}/loupan/"> ${_city.cityName }楼盘</a>&gt;<a href="${ctx}/${_city.cityPinyinAbbr}/loupan/${info.groupId}/">${buildName}</a>&gt;<span>楼盘图片</span>
				</div>
			</section>
